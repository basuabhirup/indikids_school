...
export function Hero() {
  return (
    <section id="home" className="relative min-h-[90vh] flex items-center overflow-hidden bg-gradient-to-br from-yellow-50 via-white to-blue-50">
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10 grid lg:grid-cols-2 gap-12 items-center">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white border border-gray-200 shadow-sm mb-4">
            ...
            <span className="text-sm font-semibold text-gray-600">Admission Open for Session 2026</span>
          </div>
          ...
          <p className="text-lg md:text-xl text-gray-600 max-w-xl mx-auto lg:mx-0 leading-relaxed">
            A Next Generation English Medium Primary School nurturing little angels from a homely atmosphere to a fearless learning environment.
          </p>
          
          <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start pt-4">
            <Button size="lg" className="bg-primary hover:bg-primary/90 text-white rounded-full px-8 h-14 text-lg font-bold shadow-xl shadow-primary/20 transition-all hover:scale-105" onClick={() => document.querySelector('#fees')?.scrollIntoView({ behavior: 'smooth' })}>
              Apply for 2026
              <ArrowRight className="w-5 h-5 ml-2" />
            </Button>
            <Button size="lg" variant="outline" className="rounded-full px-8 h-14 text-lg font-bold border-2 border-slate-200 text-slate-700 hover:bg-slate-50 hover:text-primary transition-all" onClick={() => document.querySelector('#about')?.scrollIntoView({ behavior: 'smooth' })}>
              Learn More
            </Button>
          </div>
        </motion.div>
